...
use App\ProceedingsParents;
use App\User;
use App\ConfigurationParameter;
use Illuminate\Support\Facades\Auth;

class ProceedingsParentsController extends Controller
{
    ...
    {   
        ...
        if (!empty($file)) {
            ...
            // file with path
            $filePath = url('uploads/actas/padres/' . $fileName . "." . $extension);
            //Move Uploaded File
            $destinationPath = 'uploads/actas/padres/';
    ...
    public function uploadFileUpdate(Request $request, int $id)
    {
        ...
        if (!empty($file)) {
            ...
            $fileSearch = ProceedingsParents::where('id', $id)->first();
            $arrayNames = explode("/", $fileSearch->url);
            $nameArchive = end($arrayNames);
            //se elimina el archivo anterior
            $filenameDelete = 'uploads/actas/padres/' . $nameArchive;
            unlink($filenameDelete);
            //se añade el nuevo archivo
            if ($file->move($destinationPath, $fileName . "." . $extension)) {
                $file = ProceedingsParents::where('id', $id)->update([
                    'url' => $filePath,
                ]);
                return "ok";
            }
    ...
    public function uploadFileUpdateSign(Request $request, int $id)
    {
        ...
        if (!empty($file)) {
            ...
            $fileName = strtr($fileName_1, " ", "_");
            // file with path
            $filePath = url('uploads/actas/padres/' . $fileName . "_FIRMADA" . "." . $extension);
            //Move Uploaded File
            $destinationPath = 'uploads/actas/padres/';
            //se añade el nuevo archivo
    ...
    public function indexProceedings()
    {
        ...
        $array=[];
        if($user->type_user==1){
            $proceedings=ProceedingsParents::all();
        }else if($user->type_user==5){
            $proceedings=ProceedingsParents::where('id_padre', $user_id)->get();
        }else if($user->type_user==2){
            $proceedings=ProceedingsParents::where('id_tutor', $user_id)->get();
        }
        foreach ($proceedings as $key => $proceeding) {
        
            if(isset($proceeding->id_padre)){
                $name_parent=User::where('id', $proceeding->id_padre)->get();
            }else{
                $name_parent="No hay padre registrado";
            }

            $proceeding->parent=$name_parent[0]->name;
    ...
    {
        $file = ProceedingsParents::where('id', $id)->first();

        $arrayNames = explode("/", $file->url);
        $nameArchive = end($arrayNames);
        $filename = '../../../uploads/actas/padres/' . $nameArchive;
    ...
    public function updateViewed(int $id)
    {
        $user = Auth::user();
        $user_id = $user->id;
        $file = ProceedingsParents::where('id', $id)->where('id_padre', $user_id)->update([
            'viewed' => 1,
        ]);
    ...
    public function downloadProceedings(int $id)
    {
        ...
        $arrayNames = explode("/", $fileSearch->url);
        $nameArchive = end($arrayNames);
        $pathtoFile = 'uploads/actas/padres/' . $nameArchive;
