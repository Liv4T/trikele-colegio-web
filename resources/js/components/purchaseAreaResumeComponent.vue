                        ...
                        <td>
                          <div class="row align-items-center">
                            <div class="col-4 col-md-2 div-plan-icon">
                              <img v-if="current_area" :src="current_area.icon" alt="incono" />
                            </div>
                            <div class="col-8 col-md-6 div-plan-title">
                              <h4>{{ current_area.name }}</h4>
                              <span>Grado {{ current_area.classroom.name }}</span>
                            </div>
...
export default {
  ...
  data() {
    return {
      ...
      voucher_code: "",
      voucher_data: null,
      current_area: { classroom: { name: "" }, credits_quantity: 0, area: { icon: "" }, plan_price: { presentation_url: "" } },
    };
  },
  methods: {
    fillWidthCalculate() {
    ...
    getArea() {
      return new Promise((resolve, reject) => {
        axios.get(`/api/area/${this.area_id}`).then(
          (response) => {
            this.current_area = response.data;
            resolve();
          },
          (e) => reject(e)
        );
    ...
    PayEvent() {
      ...
      location.href = `/compra/plan/${this.plan_type}/area/${this.area_id}/ingresar/p/${encodeURI(window.btoa(JSON.stringify(model)))}`;
