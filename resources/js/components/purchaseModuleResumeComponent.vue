                        ...
                        <td>
                          <div class="row align-items-center">
                            <div class="col-4 col-md-2 div-plan-icon">
                              <img :src="current_module.area.icon" alt="incono" />
                            </div>
                            <div class="col-8 col-md-6 div-plan-title">
                              <h4>{{ current_module.driving_question }}</h4>
                              <span>Grado {{ current_module.classroom.name }}</span>
                            </div>
                        ...
                        <td colspan="2">
                          <div class="row align-items-center">
                            <div class="col-3 col-md-2 div-plan-icon">
                              <img :src="current_module.area.icon" alt="icono" />
                            </div>
...
export default {
  ...
  data() {
    return {
      ...
      voucher_code: "",
      voucher_data: null,
      current_module: { classroom: { name: "" }, credits_quantity: 0, area: { icon: "" }, plan_price: { presentation_url: "" } },
    };
  },
  methods: {
    fillWidthCalculate() {
    ...
    getModule() {
      return new Promise((resolve, reject) => {
        axios.get(`/api/module/${this.module_id}`).then(
          (response) => {
            this.current_module = response.data;
            resolve();
          },
          (e) => reject(e)
        );
    ...
    PayEvent() {
      ...
      location.href = `/compra/plan/${this.plan_type}/modulo/${this.module_id}/ingresar/p/${encodeURI(window.btoa(JSON.stringify(model)))}`;
