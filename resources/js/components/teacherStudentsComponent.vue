<template>
  <div class="back row justify-content-center">
              ...
              <table class="table table-hover center">
                ...
                <tbody v-if="students.length > 0">
                  <tr v-for="(student, k_student) in students" v-bind:key="k_student">
                    ...
                    <td>{{ student.score }} / {{ student.score_base }}</td>
                    <td>
                    ...
                    <td>
                      <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalConection" v-on:click="getConection(student.user_id)">Última Conexión</button>
                    </td>
                  </tr>
                </tbody>
                <tbody v-if="students.length == 0">
    ...
    <div class="modal fade" id="modalConection" data-backdrop="static" data-keyboard="false">
        ...
        <div class="modal-content">
          ...
          <div class="card">
            <div class="card-body text-center">
              <h3 class="mobile-popup">Últimas 5 conexiones</h3>
              <h4 class="mobile-popup2" v-for="(item, key3) in conectionStudent" :key="key3">{{ item.created_at }}</h4>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    saveNote() {
      if (this.id_assign_note === null) {
        ...
        axios
          .put(`/AssignNote/${this.id_assign_note}`, {
            id_bimestre: this.asignNote === "parcial" ? this.idbimestre : null,
            id_student: this.data.id_student,
            id_area: this.data.id_area,
            class: this.data.class,
    ...
    getConection(idStudent) {
      axios.get(`/getConectionbyId/${idStudent}`).then((response) => {
        this.conectionStudent = response.data;
      });
    },
    deleteNote(id) {
